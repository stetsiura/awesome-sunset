...
function sunset_posted_meta()
{
    ...
    $output .= '<a href="'.esc_url(get_category_link($category->term_id)).'" alt="'.esc_attr('Просмотреть все записи в %s', $category->name).'">'.esc_html($category->name).'</a>';
    ++$i;
    endforeach;
    endif;

    return '<span class="posted-on">Опубликовано <a href="'.esc_url(get_permalink()).'">'.$posted_on.'</a> назад</span> / <span class="posted-in">'.$output.'</span>';
}
...
function sunset_posted_footer($onlyComments = false)
{
    ...
    if (comments_open()) {
        if ($comments_num == 0) {
            ...
            $comments = $comments_num.__(' Комментарии');
        } else {
        ...
        $comments = __('Комментарии закрыты');
    }
...
{
    if (is_single()) {
        $content .= '<div class="sunset-shareThis"><h4>Поделиться</h4>';
        ...
        $twitter = 'https://twitter.com/intent/tweet?text=Привет! Прочтите это: '.$title.'&amp;url='.$permalink.$twitterHandler.'';
